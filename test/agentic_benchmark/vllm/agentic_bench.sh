...
wait_for_ready() {
    local TIMEOUT=1200
    local START=$SECONDS
    until curl -sf -o /dev/null http://127.0.0.1:8002/health; do
...
stop_server() {
    if [[ -n "$SERVER_PID" ]] && kill -0 "$SERVER_PID" 2>/dev/null; then
        echo "Stopping vllm serve (pgid $SERVER_PID)..."
...
# Preflight: bail out if port 8002 is already in use
wait_for_port_free 8002
